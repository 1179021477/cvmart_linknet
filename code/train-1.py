...
net=LinkNet(2)
#net.load_state_dict(torch.load("/project/.cache/torch/hub/checkpoints/resnet18-f37072fd.pth"))#, map_location='cpu'))
net.cuda()
criterion=nn.CrossEntropyLoss().cuda()

def poly_lr_scheduler(epoch, num_epochs=50, power=0.9):
    if (1- epoch/num_epochs)**power <= 0.0001:
        return 0.0001
    return (1 - epoch/num_epochs)**power

#scheduler = LambdaLR(optimizer, lr_lambda=poly_lr_scheduler)
optimizer = torch.optim.SGD(net.parameters(), lr=1e-2,momentum=0.9, weight_decay=0.0005)
...
for epoch in range(0, epochs):
    ...
    for i, (input, lable) in enumerate(train_loader):
        ...
        if i % 100 ==0:
            ...
            print("model save!!!!")
    scheduler.step()
    #cur_lr = poly_lr_scheduler(epoch)
    #print("cur_lr:",cur_lr)
